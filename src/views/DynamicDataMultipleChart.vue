...
const option = ref({
  title: {
    text: "Dynamic Data Multiple Chart",
    left: "left",
  },
  tooltip: {
    trigger: "axis",
...
onMounted(() => {
  getFirstData("-60s", "east", eastData);
  getFirstData("-60s", "west", westData);
  getFirstData("-60s", "south", southData);
  getFirstData("-60s", "north", northData);
...
function getNewData(location, obj) {
  let newData = chartApi("-2s", location);
  // console.log(firstData);
  newData
    .then((res) => {
      if (res.length > 0) {
